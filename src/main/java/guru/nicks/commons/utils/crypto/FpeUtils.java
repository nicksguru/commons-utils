...
public class FpeUtils {
    ...
    /**
     * Creates a sequence encryptor that uses the FF1 algorithm for format-preserving encryption.
     *
     * @param nextValueSupplier  supplier for the next sequence value (e.g., from a database sequence)
     * @param zeroPadValueDigits number of digits to which the sequence value will be zero-padded (must be at least 6)
     * @param key                encryption key for FF1
     * @param tweak              tweak (like IV for AES) for FF1
    ...
    public static class Ff1SequenceEncryptor implements SequenceEncryptor {
        ...
        /**
         * Constructor. For details, see {@link #createFf1SequenceEncryptor(Supplier, int, String, String)}.
         */
        @ConstraintArguments
        Ff1SequenceEncryptor(Supplier<Long> nextValueSupplier, int zeroPadValueDigits, String key, String tweak) {
            ...
            checkNotBlank(key, _FpeUtils_Ff1SequenceEncryptorArgumentsMeta.KEY.name());
            checkNotBlank(tweak, _FpeUtils_Ff1SequenceEncryptorArgumentsMeta.TWEAK.name());

            // see PADDED_INPUT comment
            if (zeroPadValueDigits < MIN_ZERO_PAD_DIGITS) {
                throw new IllegalArgumentException("zeroPadValueDigits must be at least " + MIN_ZERO_PAD_DIGITS);
            }
            ...
            fpe = new FF1(key.getBytes(StandardCharsets.UTF_8), tweak.getBytes(StandardCharsets.UTF_8),
                    0, 0, 10, "0123456789");
            ...
            do {
                ...
                if (nextSeqValue.equals(prevSeqValue)) {
                    throw new IllegalStateException(
                            "Sequence value must be different from the previous one to fix all-zero orderId");
                }
