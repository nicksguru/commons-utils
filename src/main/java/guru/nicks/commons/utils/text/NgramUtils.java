...
public class NgramUtils {
     ...
     * @return ngrams (modifiable collection, max. {@link NgramUtilsConfig#getMaxNgramCount()} elements to avoid memory
     *         overflow, prefix ngrams go first - they have precedence before collection truncation)
     * @see #createWeightedNgrams(Set, NgramUtilsConfig)
     */
    ...
    /**
     * Creates ngrams for pre-tokenized unique words, each carrying the priority tier it belongs to: prefix ngrams are
     * high-priority, infix ngrams low-priority. This is the weighted counterpart of
     * {@link #createNgrams(Set, Mode, NgramUtilsConfig)} with {@link Mode#ALL}: the ngram sequence (ordering, dedup,
     * caps) is identical, only the phase membership is retained per ngram so that callers can, for example, emit
     * weighted tsvector data. A ngram generated by both phases (a prefix of one word, an infix of another) keeps the
     * high-priority tier because the prefix phase runs first.
     *
     * @param uniqueWords unique words to create ngrams for, <b>already lowercased (and with accents reduced, if</b>
     *                    {@link NgramUtilsConfig#isReduceAccents()} is on)
     * @param config      configuration
     * @return weighted ngrams in creation order (max. {@link NgramUtilsConfig#getMaxNgramCount()} items), prefix ngrams
     *         first
     * @see #createNgrams(String, Mode, NgramUtilsConfig)
     */
    public static SequencedSet<WeightedNgram> createWeightedNgrams(Set<String> uniqueWords, NgramUtilsConfig config) {
        // hoisted once per call - the infix loop below compares the running size against it
        int maxNgramCount = config.getMaxNgramCount();

        SequencedSet<String> prefixNgrams = createPrefixNgrams(uniqueWords, config);
        SequencedSet<WeightedNgram> weightedNgrams = LinkedHashSet.newLinkedHashSet(prefixNgrams.size() * 2);

        for (String ngram : prefixNgrams) {
            weightedNgrams.add(new WeightedNgram(ngram, true));
        }

        // prefix phase saturated the cap - infix ngrams would be discarded entirely, so skip generating them;
        // otherwise infix ngrams fill the remaining slots (an infix ngram already emitted by the prefix phase is
        // skipped - it has already claimed the high-priority tier)
        if (prefixNgrams.size() < maxNgramCount) {
            for (String ngram : createInfixNgrams(uniqueWords, config)) {
                if (weightedNgrams.size() >= maxNgramCount) {
                    break;
                }

                if (!prefixNgrams.contains(ngram)) {
                    weightedNgrams.add(new WeightedNgram(ngram, false));
                }
            }
        }
    ...
    /**
     * An ngram plus the priority tier it was generated in: prefix ngrams (and, in the full FTS pipeline, short words)
     * are high-priority, infix ngrams low-priority. Carried through to the weighted tsvector format as weight {@code A}
     * (high) or {@code B} (low), so that prefix matches rank above infix ones.
     *
     * @param ngram        the ngram
     * @param highPriority whether the ngram belongs to the high-priority (prefix) tier
     */
    public record WeightedNgram(
