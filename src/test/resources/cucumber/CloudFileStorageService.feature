...
Feature: CloudFileStorageService
  ...
  Scenario: Saving a file to cloud storage
    Given a file with content "Hello, World!" and filename "test.txt"
  ...
  Scenario: Saving a file with metadata
    Given a file with content "Test content" and filename "metadata-test.txt"
    And content type "text/plain"
    And user ID "user456"
    And metadata
      | key        | value       |
  ...
  Scenario: Finding a file by filename
    Given a file with filename "findme.txt" exists in storage
    When the file is found by filename
    Then the file should be returned
    And no exception should be thrown

  Scenario: Finding a file by ID
    Given a file with ID "file-123" exists in storage
    When the file is found by ID
    Then the file should be returned
    And no exception should be thrown

  Scenario: Getting a file that doesn't exist
    Given a file with filename "nonexistent.txt" does not exist in storage
  ...
  Scenario: Getting file content
    Given a file with ID "content-123" and content "File content" exists in storage
  ...
  Scenario: Listing files in a directory
    Given files exist in directory "test-dir/"
  ...
  Scenario Outline: Saving files with different content types
    Given a file with content "<content>" and filename "<filename>"
